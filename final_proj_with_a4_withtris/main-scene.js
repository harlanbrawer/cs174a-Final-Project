...
constructor(context, control_box) {
    ...
    this.materials =
        ...
        arena_shade1: context.get_instance(Phong_Shader).material(Color.of(0, 0.8, 0.6, 1), {ambient: 1}, {diffusivity: 1}, {specularity: 1}),
            ...
            text_image: context.get_instance(Phong_Shader).material(Color.of(1, 1, 0, 1), {ambient: 1, diffusivity: 0, specularity: 0, texture: context.get_instance( "assets/text.png", false)}),
        };
...
display(graphics_state) {
    ...
    if(t >= -19 && t < -1 && !this.skip_intro){
        ...
        let line_transformation = Mat4.translation([0, -1, 0]);
        for(let line of multi_line_string) {
            this.shapes.text1.set_string(line);
            if(t >= -2 && t < -1){
                this.shapes.text1.draw(graphics_state, text_transformation, this.materials.text_image.override({color: Color.of(0, 1, 0, -t-1)}));
            } else {
                this.shapes.text1.draw(graphics_state, text_transformation, this.materials.text_image);
            }
            text_transformation = line_transformation.times(text_transformation);
        }
    }
    ...
    if(t >= 0) {
        ...
        if (this.lose == 0) {
            this.shapes.tri.draw(graphics_state, ball_transform.times(Mat4.translation([.8,-.5,0])).times(Mat4.scale([1,.7,1])), this.materials.ball_shade);
            this.shapes.tri.draw(graphics_state, ball_transform.times(Mat4.translation([-.8,-.5 ,0])).times(Mat4.scale([1,.7,1])).times(Mat4.rotation(Math.PI *2/ 4, [0,0,1])), this.materials.ball_shade);
            this.shapes.tri.draw(graphics_state, ball_transform.times(Mat4.translation([0,1.4 ,0])).times(Mat4.scale([.7,1,1])).times(Mat4.rotation(- Math.PI *3/ 4, [0,0,1])), this.materials.ball_shade);
        } else {
            this.shapes.tri.draw(graphics_state, ball_transform.times(Mat4.translation([.8 + 1,-.5,0])).times(Mat4.scale([1,.7,1])), this.materials.ball_shade);
            this.shapes.tri.draw(graphics_state, ball_transform.times(Mat4.translation([-.8 - 1,-.5 ,0])).times(Mat4.scale([1,.7,1])).times(Mat4.rotation(Math.PI *1/ 4, [0,0,1])), this.materials.ball_shade);
            this.shapes.tri.draw(graphics_state, ball_transform.times(Mat4.translation([0 + 2,1.4 ,0])).times(Mat4.scale([.7,1,1])).times(Mat4.rotation(- Math.PI *0/ 4, [0,0,1])), this.materials.ball_shade);
        }
